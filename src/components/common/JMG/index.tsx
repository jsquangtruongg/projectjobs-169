...
import { Expo, Quad } from "gsap/all";
// Đảm bảo plugin được đăng ký
...
const AnimatedFace = ({ emailRef, passwordRef }: AnimatedFaceProps) => {
  const svgContainerRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const email = emailRef.current;
    const password = passwordRef.current;
    const svg = svgContainerRef.current?.querySelector(
      ".mySVG"
    ) as SVGSVGElement | null;

    if (!email || !password || !svg) return;

    const q = (sel: string) => svg.querySelector(sel) as Element | null;
    ...
    email.addEventListener("focus", uncoverEyes);
    password.addEventListener("focus", coverEyes);
    password.addEventListener("blur", uncoverEyes);

    return () => {
      email.removeEventListener("focus", uncoverEyes);
    ...
    <div className="svgContainer">
      <div>
        ...
        >
          ...
          <clipPath id="armMask">
            <use xmlnsXlink="#armMaskPath" overflow="visible" />
          </clipPath>
